...
#' x <- 1:length(y) - 1
#' OR.outliers.rlm.plot(x, y, max.degree = 3, p = 0.01, y.min = 0, y.max = 150)
#' @export
OR.outliers.rlm.plot <- function(x, y, max.degree = 3, p = 0.05, echo = FALSE,
                                 x.breaks = NA, x.labels = NA, x.title = "",
                                 y.min = NA, y.max = NA, y.title = "") {
  n <- length(y)
  fit <- rlm.Huber.univarpoly.AICR(x = x, y = y, max.degree = max.degree, p = p)
  fitted <- fit$fitted
  resid <- fit$resid
